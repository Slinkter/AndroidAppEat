...
import com.cudpast.myeatapp.Callback.MyButtonClickListener;
...
public abstract class MySwipeHelper extends ItemTouchHelper.SimpleCallback {
    ...
    private List<MyButton> butonList;
    ...
    private GestureDetector.SimpleOnGestureListener gestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            for (MyButton buton : butonList) {
                if (buton.onClick(e.getX(), e.getY())) {
                    break;
                }
            }
            return true;
    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (buttonBuffer.containsKey(swipePosition)) {
            butonList = buttonBuffer.get(swipePosition);
        } else {
            butonList.clear();
        }

        buttonBuffer.clear();
        swipeThreshold = 0.5f * butonList.size() * buttonWidth;
        recoverySwipedItem();
    ...
    public abstract void instatialeMyButton(RecyclerView.ViewHolder viewHolder,List<MyButton> buf);
